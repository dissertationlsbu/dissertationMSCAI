...
def match_suppliers(csv_path: str) -> dict:
    ...
    with storage.connect() as conn:
        ...
        for name in suppliers:
            ...
            rec = index.get(key)
            if not rec:
                storage.set_supplier_lookup(conn, name, None)
                stats["no_match"] += 1
                continue

            number = rec.get("number")
            sme_flag, basis = classify_account_category(rec.get("accounts"))
            if not storage.company_exists(conn, number):
                storage.upsert_company(conn, {
            ...
            storage.set_supplier_lookup(conn, name, number)
            stats["matched"] += 1
